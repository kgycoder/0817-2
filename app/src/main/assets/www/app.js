...
function closeNP() {
    ...
    // Lyrics View가 켜진 채로 닫히면, 다음에 열었을 때도 그대로 남아있지 않도록 초기화
    _setLyricsView(false);
    document.getElementById('np').classList.remove('on');
    _stopLyricsTick();
}
...
function _enterNpLandscape() {
    ...
    // 세로 Lyrics View와 레이아웃이 겹치지 않도록 먼저 해제
    _setLyricsView(false);
...
function _renderLsLyrics() {
    ...
    if (LY.videoId) _applyTranslations(LY.videoId);
}
...
const LY = {
    ...
    _fetching: null,
    _dotElFs: null,
    translations: new Map() // videoId -> Map(lineIndex -> translated text)
};

/* ════════════════════════════════════════════
   APPLE MUSIC 스타일 LYRICS VIEW
   (기존 NP 전체화면 landscape 모드와는 별개의,
    세로 화면 전용 "가사 전체보기" 모드)
════════════════════════════════════════════ */
function _showLyricsViewBtn(show) {
    const btn = document.getElementById('np-lv-btn');
    if (btn) btn.style.display = show ? '' : 'none';
}

function _setLyricsView(on) {
    const np = document.getElementById('np');
    if (!np) return;
    const isOn = np.classList.contains('lyrics-view');
    if (on === isOn) return;
    np.classList.toggle('lyrics-view', on);
    document.getElementById('np-lv-btn')?.classList.toggle('on', on);
    // 레이아웃이 바뀌면 현재 재생 줄이 새 크기 기준으로 다시 중앙 정렬돼야 함
...
function toggleLyricsView() {
    if (!LY.lines.length) { toast('가사가 없습니다'); return; }
    // 가로 전체화면 모드와는 동시에 켤 수 없음
    if (NP_LS.active) _exitNpLandscape();
    _setLyricsView(!document.getElementById('np')?.classList.contains('lyrics-view'));
}

/* ── 가사 자동 번역 (외국어 가사에 한해, 원문 줄 바로 아래 작게 표시) ──
   TODO: 실제 서비스 적용 전, 아래 apiKey를 발급받은 키로 교체할 것.
   provider는 DeepL 기준으로 구현되어 있으며, 다른 번역 API로 교체 시
   _translateLyricsIfForeign() 내부의 fetch 부분만 바꾸면 된다. */
const TRANSLATE_CFG = {
    enabled: true,
    endpoint: 'https://api-free.deepl.com/v2/translate', // Pro 플랜 사용 시 https://api.deepl.com/v2/translate
    apiKey: 'YOUR_DEEPL_API_KEY_HERE', // TODO: 실제 API 키로 교체
    targetLang: 'KO'
};

// 가사 전체 텍스트에서 한글 비중이 낮으면 "외국어 가사"로 판단
function _isForeignLyrics(lines) {
    const sample = lines.map(l => l.text || '').join('');
    const letters = (sample.match(/[A-Za-z가-힣\u3040-\u30ff\u4e00-\u9fff]/g) || []).length;
    if (letters < 5) return false;
    const hangul = (sample.match(/[가-힣]/g) || []).length;
    return (hangul / letters) < 0.25;
}

async function _translateLyricsIfForeign(videoId, lines) {
    if (!TRANSLATE_CFG.enabled || !lines.length) return;
    if (!_isForeignLyrics(lines)) return;
    if (LY.translations.has(videoId)) { _applyTranslations(videoId); return; }

    const idxMap = [];
    const texts = [];
    lines.forEach((l, i) => {
        const t = (l.text || '').trim();
        // 순수 지시어([Verse], (Chorus) 등)는 번역 대상에서 제외
        if (t && !/^[\[(].*[\])]$/.test(t)) { idxMap.push(i); texts.push(t); }
    });
    ...
    try {
        ...
        params.append('target_lang', TRANSLATE_CFG.targetLang);
        const res = await fetch(TRANSLATE_CFG.endpoint, {
            method: 'POST',
            headers: {
                'Authorization': `DeepL-Auth-Key ${TRANSLATE_CFG.apiKey}`,
        ...
        const map = new Map();
        (data.translations || []).forEach((tr, i) => { map.set(idxMap[i], tr.text); });
        LY.translations.set(videoId, map);
        if (LY.videoId === videoId) _applyTranslations(videoId);
    } catch (err) {
        console.error('lyrics translate failed', err);
    }
}

// 이미 렌더링된 모든 가사 DOM(기본 패널 / PC 전체화면 / 가로모드)에 번역 줄을 덧붙임
function _applyTranslations(videoId) {
    if (LY.videoId !== videoId) return;
    const map = LY.translations.get(videoId);
    if (!map || !map.size) return;
    // el_fs(PC 전용 window-maximize 풀스크린, Android 미사용)는 고정 56px 단일행
    // 절대배치 레이아웃이라 번역 서브라인을 넣지 않음 — 기존 레이아웃 보존.
    LY.lines.forEach((line, i) => {
        const tr = map.get(i);
        if (!tr) return;
        [line.el, line.el_ls].forEach(el => {
            if (!el || el.querySelector('.np-lyric-line-tr')) return;
            const sub = document.createElement('div');
            sub.className = 'np-lyric-line-tr';
            sub.textContent = tr;
            el.appendChild(sub);
        });
    });
...
async function fetchLyrics(videoId) {
    ...
    if (local?.lyrics?.length) {
        ...
        _renderLyrics(); // 내부에서 _translateLyricsIfForeign 호출
...
function _renderLyrics() {
    ...
    _showLyricsViewBtn(true);
    if (LY.videoId) _translateLyricsIfForeign(LY.videoId, LY.lines);
}
